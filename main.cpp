...
static void water_counter_thread(void const *args)
{
	...
	{
	    water_cnt++;
			NODE_DEBUG("\n\r *****counter thread g_water_counter= %d\n\r ",g_water_counter);
			//if(gpio0==0){
			if(gpio0.read()==0){
			NODE_DEBUG("\n\r *****read GPIO0 1 \n\r ");
			}
...
void node_state_loop()
{
	...
	while(1)
	{
		if(nodeApiJoinState()==0)
		{
			...
			NODE_DEBUG("nodeApiSetDevSleepRTCWakeup\r\n");
		...
		switch(node_state)
		{
			...
			NODE_DEBUG("NODE_STATE_ACTIVE\n\r ");
			{
				...
				//NODE_DEBUG("read sensors\n\r ");
				frame_len=node_get_sensor_data(frame);

				if(frame_len==0)
				{
					NODE_DEBUG("3.node_state=NODE_STATE_LOWPOWER no data from sensors \r\n");
					node_state=NODE_STATE_LOWPOWER;
					break;
				}
